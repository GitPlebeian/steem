...
//var obj = {name:req.body.itemName, index: req.body.index, price: req.body.price,username:req.session_state.username};
   User.findOneAndUpdate({username:req.user.username},{$push:{cart: req.body.title }},function(error,user) {
     if(error){
       return res.json(null)
     }

     User.findOneAndUpdate({username:req.user.username},{$push:{price: req.body.price }},function(error,user) {
       if(error){
         return res.json(null)
       }
       var temp = [{title: req.body.title, price: req.body.price}];
...
router.get('/cart2', function (req, res) {
 User.findOne({ username: req.user.username }, function(err, user) {
   var temp = [];
   for (var i = 0; i < user.cart.length; i++) {
       temp.push({title: user.cart[i], price: user.price[i]})
   }
   return res.json(temp)
 });
...
obj = [{game:"CSGO",price:15,picture:"/public/images/CSGO.jpg",description: "Cool FPS Game", itemNumber:1},
{game:"ORCS MUST DIE 2",price:15,picture:"/public/images/ORCS.jpg",description: "Cool ORCS Game", itemNumber:2},
{game:"PUBG",price:30,picture:"/public/images/PUBG.jpg",description: "Cool BattleRoyale Game", itemNumber:3},
{game:"Rocket League",price:20,picture:"/public/images/rocketleague.jpg",description: "Cool Car Game", itemNumber:4},
{game:"StarDew Valley",price:15,picture:"/public/images/stardewvalley.jpg",description: "Cool Farm Game", itemNumber:5},
{game:"The Forest",price:25,picture:"/public/images/theforest.jpg",description: "Cool Forest Game", itemNumber:6}]
for(let i=0;i<obj.length;i++) {
    Game.findOne({game:obj[i].name}, function(err, game) {
    if (err) { return next(err); }
    if (game) {
      return;
    }
